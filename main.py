    ...
    data = {"messaging_product":"whatsapp","to":to,"type":"interactive",
            "interactive":{"type":"list","body":{"text":"Pick report"},
                           "action":{"button":"View","sections":[{
                               "title":"Reports","rows":[
                                   {"id":"today","title":"Today","description":"Sales & profit"},
...
def process_message(uid, text, mode):
    ...
    if 'reset' in tl:
        ...
        conn.commit();conn.close();create_user(uid)
        return "Reset done! What's your business name?"
    ...
    # DATE PICKER: 2025-05-17
    if re.match(r'\d{4}-\d{2}-\d{2}', tl):
        d = tl
        conn=get_conn();c=conn.cursor()
        c.execute("SELECT COALESCE(SUM(amount),0) FROM sales WHERE user_id=%s AND DATE(timestamp)=%s",(uid,d))
        sales=c.fetchone()[0]; conn.close()
        return f"Sales on {d}: ₦{sales:,}"
...
async def webhook(r:Request):
    ...
    try:
        ...
        if msg.get('type')=='interactive':
            bid=msg['interactive']['button_reply']['id'] if 'button_reply' in msg['interactive'] else msg['interactive']['list_reply']['id']
            user_modes[uid]=bid
            if bid in ['sale','restock','expense']: send_whatsapp(uid,f"Send {bid} details")
            elif bid=='reports': send_report_menu(uid)
            elif bid=='today':
                s=get_period_sales(uid,'today'); r=get_period_expenses(uid,'today','restock'); e=get_period_expenses(uid,'today','expense')
                send_whatsapp(uid,f"TODAY\nSales ₦{s:,}\nRestock ₦{r:,}\nExpenses ₦{e:,}\nProfit ₦{s-r-e:,}")
            elif bid=='yesterday':
                # simplified
                send_whatsapp(uid,"Type date as YYYY-MM-DD for custom report")
            elif bid=='week': s=get_period_sales(uid,'week'); send_whatsapp(uid,f"Week sales ₦{s:,}")
            elif bid=='stock':
                st=get_stock(uid); txt="\n".join([f"{p}: {q}" for p,q in st]) if st else "Empty"
                send_whatsapp(uid,f"Inventory:\n{txt}")
            elif bid=='sales':
                conn=get_conn();c=conn.cursor();c.execute("SELECT product,quantity,amount FROM sales WHERE user_id=%s ORDER BY timestamp DESC LIMIT 5",(uid,)); rows=c.fetchall();conn.close()
                txt="\n".join([f"{q}x {p} - ₦{a:,}" for p,q,a in rows]) or "None"
                send_whatsapp(uid,f"Last sales:\n{txt}")
            elif bid=='profit':
                s=get_period_sales(uid,'week'); r=get_period_expenses(uid,'week','restock')
                send_whatsapp(uid,f"Week Profit: ₦{s-r:,}")
            return {"status":"ok"}
        text=msg['text']['body']
