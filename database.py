...
def create_user(uid):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "INSERT INTO users (user_id, business_name, language, created_at) VALUES (%s, %s, %s, %s) ON CONFLICT (user_id) DO NOTHING",
...
def save_expense(uid, amount, desc, typ):
    conn = get_conn(); c = conn.cursor()
    c.execute(
        "INSERT INTO expenses (user_id, amount, description, expense_type, timestamp) VALUES (%s, %s, %s, %s, %s)",
...
def add_stock(uid, product, qty, cost):
    ...
    c.execute("""
        INSERT INTO inventory (user_id, product, quantity, cost_price, last_updated)
        VALUES (%s, %s, %s, %s, %s)
